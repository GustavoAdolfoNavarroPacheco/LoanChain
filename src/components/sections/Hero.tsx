  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0" aria-hidden="true">
        ...
        <div className="orb -left-32 -top-40 h-[480px] w-[480px] bg-emerald-500/20" style={{ animationDelay: "0s" }} />
        <div className="orb -right-40 top-24 h-[420px] w-[420px] bg-amber-400/10" style={{ animationDelay: "3s" }} />
        <div className="orb bottom-0 left-1/3 h-[360px] w-[360px] bg-teal-500/10" style={{ animationDelay: "5s" }} />
      </div>
      ...
      <div className="relative mx-auto max-w-7xl px-6 pb-20 pt-36 lg:pt-44">
        <div className="grid items-center gap-16 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            ...
            {/* H1 */}
            <Reveal delay={120} variant="up" duration={900}>
              <h1 className="mt-6 text-4xl font-bold tracking-tight text-white sm:text-5xl lg:text-[3.4rem] lg:leading-[1.1]">
                Tu cadena de ahorro, <span className="text-gradient">sin sorpresas</span>
              </h1>
            </Reveal>

            {/* Subtitle */}
            <Reveal delay={240} variant="up" duration={900}>
              <p className="mt-6 max-w-xl text-lg leading-relaxed text-zinc-400">
                LoanChain convierte las natilleras de tu comunidad en una app
                móvil: turnos automáticos, pagos transparentes y control total —
                con o sin internet.
              </p>
            </Reveal>

            {/* CTAs */}
            <Reveal delay={360} variant="up" duration={900}>
              <div className="mt-9 flex flex-wrap items-center gap-4">
                <a
                  href={siteConfig.links.contact}
                  className="animate-glow inline-flex items-center justify-center gap-2 rounded-full bg-emerald-500 px-7 py-3.5 text-sm font-semibold text-emerald-950 shadow-lg shadow-emerald-500/25 transition-all duration-500 ease-(--spring) hover:-translate-y-0.5 hover:bg-emerald-400 hover:shadow-emerald-400/30 hover:shadow-xl focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-400"
                >
                  Únete a la lista de espera
                </a>
                <a
                  href={siteConfig.links.howItWorks}
                  className="group inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-7 py-3.5 text-sm font-semibold text-zinc-200 backdrop-blur transition-all duration-500 ease-(--spring) hover:border-white/30 hover:bg-white/10 hover:shadow-lg focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-400"
                >
                  Ver cómo funciona
            ...
            {/* Trust badges */}
            <Reveal delay={480} variant="up" duration={800}>
          ...
          {/* Phone mockup */}
          <Reveal delay={300} variant="scale" duration={1000} className="relative">
            <PhoneMockup />
          </Reveal>
        </div>
        ...
        <Reveal delay={200} variant="up" duration={900}>
          <dl className="mt-24 grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 lg:grid-cols-4">
            {heroStats.map((stat, i) => (
              <div
                key={stat.label}
                className="flex flex-col bg-[#070b0d] p-6 transition-colors duration-500 hover:bg-[#0a1013] lg:p-7"
                style={{
                  opacity: 0,
                  animation: `count-up 0.6s var(--spring-soft) ${600 + i * 120}ms forwards`,
                }}
              >
