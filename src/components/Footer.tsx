  ...
  return (
    <footer className="relative border-t border-white/10 bg-[#040709]">
      <div className="mx-auto max-w-7xl px-6 py-16">
          ...
          <div className="grid gap-12 lg:grid-cols-[1.5fr_1fr_1.2fr]">
            <div>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {["Android", "iOS", "Offline-First"].map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-medium text-zinc-400 transition-all duration-300 hover:border-emerald-400/20 hover:bg-emerald-400/5 hover:text-emerald-300"
                  >
                    {tag}
                  </span>
                ))}
                ...
                <ul className="mt-4 space-y-2.5">
                  {col.links.map((link) => (
                    <li key={link.href}>
                      <a
                        href={link.href}
                        className="text-sm text-zinc-500 transition-colors duration-300 hover:text-emerald-300"
                      >
                        {link.label}
                      </a>
                    </li>
        ...
        <Reveal delay={100} variant="up" duration={700}>
